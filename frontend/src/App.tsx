...
const formatStepThought = (thought: string): string => {
  if (!thought) return '';
  if (thought.includes('Gemini LLM unavailable')) {
    const match = thought.match(/Derived search keywords:.*?$/i);
    if (match) {
      return `Intent parsed using deterministic fallback. ${match[0]}`;
    }
    return 'Intent parsed using deterministic fallback heuristic parser.';
  }
  return thought;
};
...
export default function App() {
  ...
  // Growth Offer Decisions State (Upsell / Cross-Sell)
  const [offerDecisions, setOfferDecisions] = useState<Record<string, {
  ...
  const handleGrowthInteract = async (offerType: string, action: string, productId: string, baseProductId: string, quantity: number) => {
    const offerKey = `${offerType}_${productId}`;
    if (offerDecisions[offerKey]?.status === 'accepted' || offerDecisions[offerKey]?.status === 'declined') {
      return;
    }

    try {
      const res = await fetch('/api/growth/interact', {
        method: 'POST',
      ...
      if (data.status === 'ERROR') {
        setOfferDecisions(prev => ({
          ...prev,
          [offerKey]: { status: 'error', error: data.message }
        }));
        ...
        setSteps(prev => [
          ...prev,
          {
            ...
            title: offerType === 'cross_sell' ? "Cross-Sell Policy Blocked" : "Upsell Policy Blocked",
            thought: `Offer rejected by Policy Engine: ${data.message}`,
            action: "growth_offer_rejected_by_policy",
            status: "REJECTED",
      ...
      if (action === 'accept') {
        ...
        const prodName = data.product?.name || "Add-on Item";
        const totalStr = data.total_amount ? `₹${data.total_amount.toLocaleString('en-IN')}` : '';

        setSteps(prev => [
          ...prev,
          {
            step_number: prev.length + 1,
            title: offerType === 'cross_sell' ? "Cross-Sell Added to Cart" : "Upsell Upgrade Accepted",
            thought: `Added '${prodName}' to order proposal. Updated cart total: ${totalStr}.`,
            action: "growth_offer_accepted",
            status: "SUCCESS",
            data: {
        ...
        if (data.verification?.status === 'HITL_REQUIRED') {
          setPendingHitl({
            sessionId: activeSessionId,
            proposal: data.proposal,
            verifiedTotal: data.total_amount,
            hitlToken: data.hitl_token || data.verification.hitl_token || ""
          });
        }
      } else {
        setOfferDecisions(prev => ({
        ...
        setSteps(prev => [
          ...
          {
            step_number: prev.length + 1,
            title: offerType === 'cross_sell' ? "Cross-Sell Declined" : "Upsell Declined",
            thought: `Declined offer for '${prodName}'. Proceeding with original order.`,
            action: "growth_offer_declined",
            status: "COMPLETED",
  ...
  const handleRunAgent = async (customGoal?: string, customBudget?: number) => {
    ...
    if (customGoal) setPromptGoal(customGoal);
    if (customBudget !== undefined) setMaxBudget(customBudget);

    setActiveTab('product'); // Always stay on Product view to show the execution view
    setIsRunning(true);
    setSteps([]);
    setPendingHitl(null);
    setOfferDecisions({});
    ...
    try {
        ...
        for (const line of lines) {
          if (line.startsWith("data: ")) {
            ...
            try {
              ...
              setSteps(prev => {
                const existingIdx = prev.findIndex(s => s.step_number === step.step_number);
                if (existingIdx >= 0) {
                  const next = [...prev];
                  next[existingIdx] = {
                    ...next[existingIdx],
                    ...step,
                    data: step.data && Object.keys(step.data).length > 0 ? step.data : next[existingIdx].data
                  };
                  return next;
                }
                const updatedPrev = prev.map(s => s.status === 'IN_PROGRESS' ? { ...s, status: 'COMPLETED' } : s);
                return [...updatedPrev, step];
              });
      ...
      setIsRunning(false);
      setSteps(prev => prev.map(s => s.status === 'IN_PROGRESS' ? { ...s, status: 'COMPLETED' } : s));
      fetchAuditLogs();
      fetchCatalog();
    }
  };
  ...
  // Determine Final Execution Status for the Product View Execution Screen
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 flex flex-col font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* Top Clean Persistent Navigation Bar */}
      <header className="border-b border-slate-200 bg-white sticky top-0 z-40 px-4 sm:px-8 py-3.5 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          
          {/* Brand & Identity */}
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActiveTab('product')}>
      ...
      {/* Main Layout: Compact Left Navigation + Dedicated Category View */}
      <div className="flex-1 max-w-7xl w-full mx-auto flex flex-col md:flex-row px-4 sm:px-6 lg:px-8 gap-4 md:gap-6 lg:gap-8">
        
        {/* Persistent Compact Left Navigation Sidebar (No serial numbers) */}
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all whitespace-nowrap text-left cursor-pointer w-full ${
                    isActive 
        ...
        {/* Dedicated Main Content Area (Renders ONLY the Active Category View) */}
        <main className="flex-1 min-w-0 py-3 md:py-6">
          
          {/* ========================================================= */}
          {/* 1. PRODUCT VIEW: Hero + Goal Input OR Dedicated Execution */}
          {/* ========================================================= */}
          {activeTab === 'product' && (
            <div className="space-y-8 animate-reveal">
              
              {/* Hero Section: Exact ColdStream-Style 2-line Heading */}
              <div className="text-center max-w-2xl mx-auto pt-2 pb-2">
                ...
                <p className="text-sm sm:text-base text-slate-600 leading-relaxed font-normal max-w-xl mx-auto animate-hero-desc mt-3">
              ...
              {/* View Toggle: Initial Intent Form vs Live Execution View */}
              {steps.length === 0 && !isRunning ? (
                
                /* --- A. Agent Purchase Intent Card --- */
                <div className="max-w-2xl mx-auto bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-[11px] text-slate-400">Click to run scenario</span>
                    </div>
                ...
                /* --- B. DEDICATED PURCHASE EXECUTION / RESULT VIEW --- */
                <div className="max-w-3xl mx-auto space-y-4 animate-reveal">
                  
                  {/* Execution Header & Status Ribbon */}
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3.5 mb-3.5 border-b border-slate-100 gap-3">
                      <div>
                        <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Autonomous Commerce Pipeline</span>
                        <h2 className="text-base font-bold text-slate-900">Purchase Execution &amp; Settlement Trace</h2>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {activeSessionId && (
                          <span className="font-mono text-xs text-slate-600 bg-slate-50 border border-slate-200 px-2 py-1 rounded-md">
                            {activeSessionId}
                          </span>
                        )}
                        ...
                        <button
                          onClick={handleStartNewPurchase}
                          disabled={isRunning}
                          className="px-3 py-1.5 bg-white hover:bg-slate-50 disabled:opacity-50 text-slate-700 text-xs font-semibold rounded-lg border border-slate-300 shadow-sm flex items-center gap-1.5 transition-all hover:border-slate-400 cursor-pointer ml-1"
                          title="Clear and start a fresh purchase"
                        >
                    ...
                    {/* Intent & Budget Summary */}
                    <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-slate-700">
                      <div>
                        <span className="text-slate-400 font-medium">Goal Intent: </span>
                        <strong className="text-slate-900">{promptGoal}</strong>
                      </div>
                      <div className="sm:text-right shrink-0">
                        <span className="text-slate-400 font-medium">Budget Cap: </span>
                        <strong className="text-slate-900">₹{maxBudget.toLocaleString('en-IN')}</strong>
                      </div>
                    </div>

                    {/* Pipeline Stage Bar */}
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 mt-3 pt-3 border-t border-slate-100 text-[11px] font-medium text-center">
                      <div className={`p-1.5 rounded-lg border ${
                        steps.length >= 1 ? 'bg-emerald-50 text-emerald-700 border-emerald-200 font-semibold' : 'bg-slate-50 text-slate-400 border-slate-200'
                      }`}>
                        1. Intent
                      </div>
                      <div className={`p-1.5 rounded-lg border ${
                        steps.length >= 2 ? 'bg-emerald-50 text-emerald-700 border-emerald-200 font-semibold' : isRunning ? 'bg-blue-50 text-blue-700 border-blue-200 font-semibold' : 'bg-slate-50 text-slate-400 border-slate-200'
                      }`}>
                        2. Discovery
                      </div>
                      <div className={`p-1.5 rounded-lg border ${
                        steps.length >= 3 ? 'bg-emerald-50 text-emerald-700 border-emerald-200 font-semibold' : 'bg-slate-50 text-slate-400 border-slate-200'
                      }`}>
                        3. Policy Check
                      </div>
                      <div className={`p-1.5 rounded-lg border ${
                      ...
                      <div className={`p-1.5 rounded-lg border ${
                        steps.some(s => s.title?.toLowerCase().includes('payment')) ? 'bg-emerald-50 text-emerald-700 border-emerald-200 font-semibold' : 'bg-slate-50 text-slate-400 border-slate-200'
                      }`}>
                        5. Payment
                      </div>
                      <div className={`p-1.5 rounded-lg border ${
                        isSuccess ? 'bg-emerald-50 text-emerald-700 border-emerald-200 font-semibold' : isBlocked ? 'bg-rose-50 text-rose-700 border-rose-200 font-semibold' : 'bg-slate-50 text-slate-400 border-slate-200'
                      }`}>
                        6. Settlement
                      </div>
                    </div>
                  ...
                  {/* HITL Sign-off Card (When Approval is Required) */}
                  {pendingHitl && (
                    <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-5 shadow-sm animate-reveal">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            <div className="flex items-center space-x-2">
                              <h3 className="font-bold text-slate-900 text-base">Human-in-the-Loop Sign-off Required</h3>
                              <span className="text-xs px-2 py-0.5 rounded bg-amber-200 text-amber-900 font-semibold">
                                High-Value Threshold Gate
                              </span>
                            </div>
                            <p className="text-xs text-slate-600 mt-1 max-w-xl leading-relaxed">
                              This purchase total of <strong className="text-slate-900">₹{pendingHitl.verifiedTotal.toLocaleString('en-IN')}</strong> exceeds the autonomous pre-authorization threshold of ₹{policy.auto_approve_limit.toLocaleString('en-IN')}. Verify Merchant details to proceed.
                            </p>
                            <div className="flex items-center space-x-4 mt-2 text-xs font-medium text-slate-700">
                        ...
                        <div className="flex items-center space-x-2.5 sm:self-center">
                          <button
                            ...
                            className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl flex items-center justify-center space-x-1.5 shadow-sm transition-all cursor-pointer"
                          >
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
                      ...
                      {steps.map((step, idx) => (
                        <div 
                          ...
                          className={`p-3.5 rounded-xl border transition-all ${
                            step.status === 'SUCCESS' || step.status === 'COMPLETED' ? 'bg-emerald-50/40 border-emerald-200' :
                            step.status === 'REJECTED' || step.status === 'FAILED' ? 'bg-rose-50/40 border-rose-200' :
                        ...
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded-full ${
                              step.status === 'SUCCESS' || step.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800' :
                              step.status === 'REJECTED' || step.status === 'FAILED' ? 'bg-rose-100 text-rose-800' :
                              step.status === 'PENDING_APPROVAL' ? 'bg-amber-100 text-amber-800' :
                              step.status === 'RECOVERING' ? 'bg-purple-100 text-purple-800' :
                              step.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-800 animate-pulse' :
                          ...
                          <p className="text-xs text-slate-700 leading-relaxed font-normal pl-6">{formatStepThought(step.thought)}</p>
                          
                          {/* Contextual Upsell Option */}
                          {step.data?.upsell_candidate && (() => {
                            const upsellKey = `upsell_${step.data.upsell_candidate.upsell_product.id}`;
                            const decision = offerDecisions[upsellKey];
                            return (
                              <div className="mt-2.5 ml-6 p-2.5 rounded-lg bg-white border border-blue-200 text-xs space-y-1.5 shadow-sm">
                                ...
                                {decision?.status === 'accepted' ? (
                                  <div className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200 w-fit">
                                    <Check className="h-3.5 w-3.5" /> Upgrade Accepted (+₹{step.data.upsell_candidate.price_delta.toLocaleString('en-IN')})
                                  </div>
                                ) : decision?.status === 'declined' ? (
                                  <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200 w-fit">
                                    <span>Upsell Offer Declined</span>
                                  </div>
                                ) : decision?.status === 'error' ? (
                                  <div className="text-xs font-medium text-rose-700 bg-rose-50 p-2 rounded-md border border-rose-200">
                                    {decision.error}
                                  </div>
                                ) : (
                                  <div className="flex items-center space-x-2 pt-0.5">
                                    <button
                                      ...
                                      className="px-2.5 py-1 bg-[#0c83fe] hover:bg-[#0066ff] text-white rounded text-xs font-semibold transition-colors cursor-pointer flex items-center gap-1"
                                    >
                          ...
                          {step.data?.cross_sell_candidate && (() => {
                            const crossSellKey = `cross_sell_${step.data.cross_sell_candidate.cross_sell_product.id}`;
                            const decision = offerDecisions[crossSellKey];
                            return (
                              <div className="mt-2.5 ml-6 p-2.5 rounded-lg bg-white border border-purple-200 text-xs space-y-1.5 shadow-sm">
                                ...
                                {decision?.status === 'accepted' ? (
                                  <div className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200 w-fit">
                                    <Check className="h-3.5 w-3.5" /> Added to Cart (+₹{step.data.cross_sell_candidate.additional_price.toLocaleString('en-IN')})
                                  </div>
                                ) : decision?.status === 'declined' ? (
                                  <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200 w-fit">
                                    <span>Cross-Sell Offer Declined</span>
                                  </div>
                                ) : decision?.status === 'error' ? (
                                  <div className="text-xs font-medium text-rose-700 bg-rose-50 p-2 rounded-md border border-rose-200">
                                    {decision.error}
                                  ...
                                  <div className="flex items-center space-x-2 pt-0.5">
                                    <button
                                      ...
                                      className="px-2.5 py-1 bg-purple-600 hover:bg-purple-700 text-white rounded text-xs font-semibold transition-colors cursor-pointer flex items-center gap-1"
                                    >
                    ...
                    {/* Bottom Execution Action Toolbar */}
                    <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2.5">
                      <button
                        onClick={handleStartNewPurchase}
                        disabled={isRunning}
                        className="px-4 py-2 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 text-xs font-semibold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer"
                      >
                      ...
                      <div className="flex items-center space-x-2">
                        <button
                          ...
                          className="px-3.5 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs font-medium rounded-lg transition-colors cursor-pointer flex items-center gap-1"
                        >
          ...
          {/* ========================================================= */}
          {/* 2. CATALOG VIEW: Merchant Inventory & MCP Catalog         */}
          {/* ========================================================= */}
          {activeTab === 'catalog' && (
            <div className="space-y-6 animate-reveal">
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-7">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-5 mb-5 border-b border-slate-100 gap-4">
                  <div>
                    <div className="flex items-center space-x-2">
                      <Database className="h-5 w-5 text-[#0052cc]" />
                      <h2 className="text-lg font-bold text-slate-900">Merchant Inventory (MCP Catalog)</h2>
                    </div>
                  ...
                  <div className="flex items-center space-x-2.5">
                    <button
                      ...
                      className="px-3 py-2 rounded-lg border border-rose-200 bg-rose-50 text-rose-700 text-xs font-semibold hover:bg-rose-100 transition-colors cursor-pointer"
                    >
                      Simulate Gateway Failure
                    </button>
                    <button 
                      onClick={resetCatalog}
                      className="text-xs text-slate-700 hover:text-slate-900 flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded-lg font-medium transition-colors cursor-pointer"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {products.map(p => (
                    <div 
                      key={p.id} 
                      className="bg-white border border-slate-200 rounded-xl p-4 hover:border-slate-300 transition-all flex flex-col justify-between"
                    >
                      <div>
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="text-base font-extrabold text-[#0c2340] whitespace-nowrap">
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-[11px] text-slate-400 truncate max-w-[100px]">{p.merchant_name}</span>
                        </div>
                        ...
                        <div className="flex items-center space-x-1.5">
                          <button
                            ...
                            className="px-2 py-1 rounded bg-slate-100 hover:bg-amber-50 text-slate-600 hover:text-amber-700 text-[11px] font-medium border border-slate-200 transition-colors cursor-pointer"
                          >
                            +Surge
                          </button>
                          <button
                            ...
                            className="px-2 py-1 rounded bg-slate-100 hover:bg-rose-50 text-slate-600 hover:text-rose-700 text-[11px] font-medium border border-slate-200 transition-colors cursor-pointer"
                          >
                            Deplete
                          </button>
                        </div>
          ...
          {/* ========================================================= */}
          {/* 3. ANALYTICS VIEW: Merchant Growth & Conversion Metrics    */}
          {/* ========================================================= */}
          {activeTab === 'analytics' && (
            <div className="space-y-6 animate-reveal">
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-7">
                <div className="flex items-center justify-between pb-5 mb-5 border-b border-slate-100">
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <h2 className="text-lg font-bold text-slate-900">Merchant Growth Analytics</h2>
                    </div>
                    <p className="text-xs text-slate-500 mt-1">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <span className="text-xl font-extrabold text-[#0c2340] mt-1 block">
                      ₹{merchantAnalytics?.gmv ? merchantAnalytics.gmv.toLocaleString('en-IN') : '0'}
                    </span>
                    <span className="text-[11px] text-emerald-600 font-medium mt-1 block">Settled on Razorpay</span>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <span className="text-[11px] text-slate-500 font-medium mt-1 block">Completed orders</span>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <span className="text-[11px] text-slate-500 font-medium mt-1 block">Autonomous buy-in</span>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <span className="text-[11px] text-slate-500 font-medium mt-1 block">AOV per transaction</span>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <span className="text-[11px] text-amber-600 font-medium mt-1 block">Upsell &amp; cross-sell lift</span>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    <span className="text-xs font-medium text-slate-500 block">Upsell Acceptance</span>
                    <span className="text-xl font-extrabold text-[#0052cc] mt-1 block">
                      {metrics?.upsell_acceptance_rate || 0}%
                    </span>
                    <span className="text-[11px] text-slate-500 font-medium mt-1 block">Contextual upgrades</span>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <span className="text-[11px] text-slate-500 font-medium mt-1 block">Add-on purchases</span>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <span className="text-[11px] text-slate-500 font-medium mt-1 block">High-value approvals</span>
                  </div>
                ...
                {merchantAnalytics && (
                  <div className="mt-4 p-3 rounded-xl bg-blue-50/60 border border-blue-100 flex items-center justify-between text-xs">
                    <div className="flex items-center space-x-2 text-slate-700">
                      <ShieldCheck className="h-4 w-4 text-[#0052cc]" />
                      <span className="font-medium">Stockout Auto-Recovery Fail-Safes:</span>
                    </div>
                    <span className="font-bold text-emerald-700 bg-emerald-100 px-2.5 py-1 rounded-md text-xs">
          ...
          {/* ========================================================= */}
          {/* 5. AUDIT LEDGER VIEW: SHA-256 Chained Event Log           */}
          {/* ========================================================= */}
          {activeTab === 'audit' && (
            <div className="space-y-6 animate-reveal">
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-7">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-5 mb-5 border-b border-slate-100 gap-3">
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <h2 className="text-lg font-bold text-slate-900">Cryptographic Audit Ledger</h2>
                    </div>
                    <p className="text-xs text-slate-500 mt-1">
